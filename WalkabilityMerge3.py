...
def renameFeature(src,featureClass,parcelsFC,workspace):
    shpfiles = os.listdir(workspace)
    try:
        for shpfile in shpfiles:
            if shpfile.lower().startswith(featureClass):
                parcelsNewName = parcelsFC + shpfile.split('.')[1]
                os.rename(shpfile, parcelsNewName, workspace)
    except:
...
if __name__ == '__main__':
    ...
    workspace = destinationsDir #+ "/" + geoDataBaseName #"C:/Users/dev/Documents/boundarysolutions/marin/Marin.gdb"
    ...
    bufferDestinations(mergedMetroDestinations,"destinationsCommercialBufferedHalf",geoDataBaseName)
    ...
    for countyfolder in countyFolderList:
        ...
        src = dataPath + countyfolder + "\\"  + fc
        parcelsFC = dataPath + countyfolder + featureClass +  countyfolder
        parcelsSHP = renameFeature(src, featureClass, parcelsFC, workspace)
        os.rename(src, parcelsFC, workspace)
        arcpy.FeatureClassToGeodatabase_conversion(parcelsSHP, outWorkspace)
        scoreParcels(parcelsFC,"destinationsCommercialBufferedHalf", outputName, geoDataBaseName)
